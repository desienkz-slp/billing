<template>
    <AppLayout title="Pelanggan Cuti">
            ...
            <div class="mb-6 flex flex-col gap-4 px-4 sm:px-0">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div class="flex items-center">
                        <svg class="w-6 h-6 sm:w-7 sm:h-7 text-amber-600 dark:text-amber-400 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <h1 class="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Pelanggan Cuti</h1>
                    </div>
                    ...
                    <table class="w-full text-xs sm:text-sm text-left text-slate-500 dark:text-slate-400 relative">
                        <thead class="sticky top-0 z-10 text-xs sm:text-sm font-semibold uppercase tracking-wider text-slate-500 bg-slate-50 dark:bg-[#1E293B] dark:text-slate-400 shadow-[0_1px_0_0_#e2e8f0] dark:shadow-[0_1px_0_0_#334155]">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3 text-center whitespace-nowrap">TGL CUTI</th>
                        ...
                        <tbody>
                            <tr v-for="customer in customers.data" :key="customer.id" class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-4 py-3 text-center text-xs sm:text-sm whitespace-nowrap">
                                    <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-400">
                                        {{ customer.leave_start ? new Date(customer.leave_start).toLocaleDateString('id-ID') : '-' }}
                                    </span>
                                </td>
                            ...
                            <tr v-if="customers.data.length === 0">
                                <td colspan="8" class="px-4 py-16 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 rounded-2xl">
                                    ...
                                    Tidak ada data pelanggan yang sedang cuti.
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <Modal :show="isRestoreModalOpen" @close="closeRestoreModal" maxWidth="md">
            <div class="p-6">
                <h3 class="text-lg font-bold text-emerald-600 dark:text-emerald-400 mb-4">Konfirmasi Restore Pelanggan</h3>
                <p class="text-sm text-slate-600 dark:text-slate-300 mb-6">
                    Apakah Anda yakin ingin me-restore <span v-if="restoreTarget">pelanggan <strong>{{ restoreTarget.name }}</strong></span><span v-else><strong>{{ selectedCustomers.length }} pelanggan</strong> yang dipilih</span> dari status cuti?
                </p>
...
watch(formFilters, (newValues) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get(route('cust.cuti.index'), newValues, { preserveState: true, replace: true });
    }, 300);
}, { deep: true });
...
const isAllSelected = computed(() => {
    return props.customers.data && props.customers.data.length > 0 && selectedCustomers.value.length === props.customers.data.length;
});
...
const executeForceDelete = () => {
    ...
    if (deleteTarget.value) {
        // Single force delete
        router.delete(route('cust.cuti.destroy', deleteTarget.value.id), {
            onSuccess: () => {
                closeDeleteModal();
        ...
        // Batch force delete
        router.post(route('cust.cuti.batch-destroy'), {
...
const executeRestore = () => {
    ...
    if (restoreTarget.value) {
        // Single restore
        router.post(route('cust.cuti.restore', restoreTarget.value.id), {}, {
            onSuccess: () => {
                closeRestoreModal();
        ...
        // Batch restore
        router.post(route('cust.cuti.batch-restore'), {
            ids: selectedCustomers.value,
        }, {
            onSuccess: () => {
                closeRestoreModal();
