        ...
        <nav class="sidebar-nav">
            ...
            <template v-if="!route().current('config.*')">
                <div class="nav-section">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_dashboard" :href="route('cust.dashboard')" class="nav-item" :class="{ 'active': route().current('cust.dashboard') }">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
                        <span class="nav-text">Dashboard</span></Link>

                    <Link v-if="isAdmin || role?.can_view_menu_pelanggan" :href="route('cust.customers.index')" class="nav-item" :class="{ 'active': route().current('cust.customers.*') }">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                        <span class="nav-text">Pelanggan</span></Link>

                    <Link v-if="isAdmin || role?.can_view_menu_pelanggan_cuti" :href="route('cust.cuti.index')" class="nav-item" :class="{ 'active': route().current('cust.cuti.*') }">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_isolir" :href="route('cust.isolir.index')" class="nav-item" :class="{ 'active': route().current('cust.isolir.*') }">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
                        <span class="nav-text">Isolir</span></Link>
                </div>

                <div class="nav-section" v-if="isAdmin || role?.can_view_menu_income || role?.can_view_menu_other_in || role?.can_view_menu_expenses || role?.can_view_menu_tax || role?.can_view_menu_recap || role?.can_view_menu_cashflow || role?.can_view_menu_statistics">
                    <div class="nav-section-title">Laporan</div>
                    
                    <Link v-if="isAdmin || role?.can_view_menu_income" :href="route('reports.income.index')" class="nav-item" :class="{ 'active': route().current('reports.income.*') }">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_other_in" :href="route('reports.other-incomes.index')" class="nav-item" :class="{ 'active': route().current('reports.other-incomes.*') }">
                        ...
                        <Link v-if="isAdmin || role?.can_view_menu_expenses" :href="route('reports.expenses.index')" class="nav-item" :class="{ 'active': route().current('reports.expenses.*') }">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                            <span class="nav-text">Expenses</span></Link>
                        <Link v-if="isAdmin || role?.can_view_menu_tax" :href="route('reports.tax')" class="nav-item" :class="{ 'active': route().current('reports.tax') }">
                        ...
                        <Link v-if="isAdmin || role?.can_view_menu_recap" :href="route('reports.total')" class="nav-item" :class="{ 'active': route().current('reports.total') }">
                        ...
                        <Link v-if="isAdmin || role?.can_view_menu_cashflow" :href="route('reports.cashflow')" class="nav-item" :class="{ 'active': route().current('reports.cashflow') }">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
                            <span class="nav-text">Cashflow</span></Link>
                        <Link v-if="isAdmin || role?.can_view_menu_statistics" :href="route('reports.statistics')" class="nav-item" :class="{ 'active': route().current('reports.statistics*') }">
                ...
                <div class="nav-section" v-if="isAdmin || role?.can_view_menu_fee || role?.can_view_menu_setoran || role?.can_view_menu_saldo || role?.can_view_menu_mitra">
                    <div class="nav-section-title">Laporan Sales</div>
                    
                    <Link v-if="isAdmin || role?.can_view_menu_fee" :href="route('sales.fee')" class="nav-item" :class="{ 'active': route().current('sales.fee*') }">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_setoran" :href="route('sales.setoran')" class="nav-item" :class="{ 'active': route().current('sales.setoran') }">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_saldo" :href="route('sales.saldo')" class="nav-item" :class="{ 'active': route().current('sales.saldo') }">
                    ...
                    <Link v-if="isAdmin || role?.can_view_menu_mitra" :href="route('sales.mitra')" class="nav-item" :class="{ 'active': route().current('sales.mitra') }">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        <span class="nav-text">Mitra</span></Link>
                </div>

                <div class="nav-section" v-if="isAdmin || role?.can_view_menu_master_paket || role?.can_view_menu_master_area">
                    <div class="nav-section-title">Master</div>
                    
                    <Link v-if="isAdmin || role?.can_view_menu_master_paket" :href="route('settings.packages')" class="nav-item" :class="{ 'active': route().current('settings.packages*') }">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 7V5a4 4 0 0 0-8 0v2"/></svg>
                        <span class="nav-text">Master Paket</span></Link>
                    <Link v-if="isAdmin || role?.can_view_menu_master_area" :href="route('settings.areas')" class="nav-item" :class="{ 'active': route().current('settings.areas*') }">
