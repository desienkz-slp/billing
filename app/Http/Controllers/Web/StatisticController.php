...
class StatisticController extends Controller
{
    public function index(Request $request)
    {
        ...
        return inertia('Reports/Statistics', compact('bulan'));
    }
        ...
        switch ($action) {
            ...
            case 'payment_daily':
                $payments = Payment::where('tenant_id', $tenantId)
                    ->whereBetween('payment_date', [Carbon::today()->subDays(29)->startOfDay(), Carbon::today()->endOfDay()])
                    ->selectRaw('DATE(payment_date) as tgl, SUM(amount) as total')
                    ->groupBy('tgl')
                    ->orderBy('tgl')
                    ->get();
            ...
            case 'payment_monthly':
                $payments = Payment::where('tenant_id', $tenantId)
                    ...
                    ->selectRaw('to_char(payment_date, \'YYYY-MM\') as bln, SUM(amount) as total')
                    ->groupBy('bln')
                    ->orderBy('bln')
                    ->get();
                return response()->json(['status' => 'success', 'data' => $payments]);
